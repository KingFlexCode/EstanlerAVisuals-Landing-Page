...
const COVER_STYLES = [
  ...
  { id: "divider", label: "Divider", description: "Dimmed text panel on the left with full-color image on the right." },
...
function GalleryPreview({ gallery, sections, photos, coverPhoto, previewMode }) {
  ...
  const titleBlock = (align = "center", color = "#fff") => <div style={{ textAlign: align, color, maxWidth: 760, textShadow: color === "#fff" ? "0 8px 24px rgba(0,0,0,0.35)" : "none" }}><div style={{ color: themeColor, fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 800, letterSpacing: "0.16em", marginBottom: "0.75rem", textTransform: "uppercase" }}>{formatDate(gallery.event_date)}</div><h1 style={{ fontFamily: typeTheme.family, fontSize: isMobile ? "2rem" : "3.1rem", fontWeight: typeTheme.weight, letterSpacing: typeTheme.spacing, lineHeight: 1, margin: 0, textTransform: typeTheme.transform }}>{gallery.title || "Untitled Gallery"}</h1>{gallery.client_name && <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, letterSpacing: "0.08em", margin: "1rem 0 0", textTransform: "uppercase" }}>{gallery.client_name}</p>}</div>;
  ...
  const renderHero = () => {
    ...
    if (coverStyle === "divider") return <section style={{ minHeight: isMobile ? 330 : 420, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1.65fr", background: "#111", position: "relative" }}><div style={{ ...fullColorImageStyle, minHeight: isMobile ? 220 : 420, position: "relative", display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? "2rem" : "3rem", boxSizing: "border-box" }}><span style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.48)" }} /><div style={{ position: "relative", zIndex: 1 }}>{titleBlock("center", "#fff")}<button type="button" style={{ marginTop: "2.2rem", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.65)", color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 10, fontWeight: 800, letterSpacing: "0.14em", padding: "0.8rem 1.35rem", textTransform: "uppercase" }}>View Gallery</button></div></div><div style={{ ...fullColorImageStyle, minHeight: isMobile ? 260 : 420 }} /></section>;
...
export default function GalleryEditor() {
  ...
  function renderCoverTemplatePreview(style) {
    ...
    if (style.id === "divider") return <div style={{ height: 86, display: "grid", gridTemplateColumns: "42% 58%", background: "#111" }}><div style={{ background: imageBackground, position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.48)" }} />{title}</div><div style={{ background: imageBackground }} /></div>;

...
const COVER_STYLES = [
  ...
  { id: "left", label: "Left", description: "Full image cover with left title." },
  { id: "novel", label: "Novel", description: "Editorial white panel with image block." },
  { id: "vintage", label: "Vintage", description: "Classic title treatment over image." },
  { id: "frame", label: "Frame", description: "Inset cover image with border." },
  { id: "stripe", label: "Stripe", description: "Minimal title with horizontal stripe." },
  { id: "divider", label: "Divider", description: "Split image layout with title overlay." },
...
function GalleryPreview({ gallery, sections, photos, coverPhoto, previewMode }) {
  ...
  const editorialLetterSpacing = typography === "editorial" ? "0.18em" : "0.02em";

  const titleBlock = (align = "center", color = "#fff") => <div style={{ textAlign: align, color, maxWidth: 760 }}><div style={{ color: themeColor, fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 800, letterSpacing: "0.16em", marginBottom: "0.75rem", textTransform: "uppercase" }}>{formatDate(gallery.event_date)}</div><h1 style={{ fontFamily: headingFont, fontSize: isMobile ? "2rem" : "3.1rem", fontWeight: typography === "modern" ? 700 : 600, letterSpacing: editorialLetterSpacing, lineHeight: 1, margin: 0, textTransform: typography === "editorial" ? "uppercase" : "none" }}>{gallery.title || "Untitled Gallery"}</h1>{gallery.client_name && <p style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, letterSpacing: "0.08em", margin: "1rem 0 0", textTransform: "uppercase" }}>{gallery.client_name}</p>}</div>;
  const imageStyle = coverUrl ? { backgroundImage: `linear-gradient(rgba(0,0,0,0.18), rgba(0,0,0,0.48)), url(${coverUrl})`, backgroundPosition: objectPosition, backgroundSize: "cover", backgroundRepeat: "no-repeat" } : { background: "linear-gradient(135deg, #1c1c1c, #3b3b3b)" };

  const renderHero = () => {
    if (coverStyle === "novel") return <section style={{ minHeight: isMobile ? 330 : 420, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "42% 58%", background: "#fff", border: "18px solid #fff", boxSizing: "border-box" }}><div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>{titleBlock("left", "#111")}</div><div style={{ ...imageStyle, minHeight: isMobile ? 260 : 384 }} /></section>;
    if (coverStyle === "journal") return <section style={{ minHeight: isMobile ? 330 : 420, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "62% 38%", background: "#fff" }}><div style={{ ...imageStyle, minHeight: isMobile ? 280 : 420 }} /><div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>{titleBlock("left", "#111")}</div></section>;
    if (coverStyle === "minimal") return <section style={{ minHeight: isMobile ? 330 : 420, background: "#fff", color: "#111", display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", boxSizing: "border-box" }}><div style={{ textAlign: "center", width: "100%", maxWidth: 520 }}><div style={{ width: isMobile ? 160 : 220, aspectRatio: "1 / 1", margin: "0 auto 1.5rem", ...imageStyle }} />{titleBlock("center", "#111")}</div></section>;
    if (coverStyle === "split") return <section style={{ minHeight: isMobile ? 330 : 420, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", background: "#101010" }}><div style={{ ...imageStyle, minHeight: isMobile ? 250 : 420 }} /><div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>{titleBlock("left", "#fff")}</div></section>;
    if (coverStyle === "divider") return <section style={{ minHeight: isMobile ? 330 : 420, display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 6, background: "#fff", padding: 6, boxSizing: "border-box", position: "relative" }}><div style={{ ...imageStyle, minHeight: isMobile ? 250 : 408 }} /><div style={{ ...imageStyle, minHeight: isMobile ? 250 : 408, filter: "saturate(0.75) brightness(0.8)" }} /><div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>{titleBlock("center", "#fff")}</div></section>;
    ...
    return <section style={{ minHeight: isMobile ? 330 : 420, ...imageStyle, display: "flex", alignItems, justifyContent, padding: isMobile ? "2rem" : "3.5rem", boxSizing: "border-box", border: coverStyle === "frame" ? "18px solid #fff" : "none", position: "relative" }}>{coverStyle === "stripe" && <span style={{ position: "absolute", left: "12%", right: "12%", top: "50%", height: 1, background: "rgba(255,255,255,0.75)" }} />}{coverStyle === "vintage" && <span style={{ position: "absolute", inset: "1.4rem", border: "1px solid rgba(255,255,255,0.55)" }} />}{titleBlock(textAlign, "#fff")}</section>;
  };
...
export default function GalleryEditor() {
  ...
  function getDropPlacement(event) {
    ...
    if (workspaceViewMode === "list") return event.clientY >= rect.top + rect.height * 0.35 ? "after" : "before";
    return event.clientX >= rect.left + rect.width * 0.35 ? "after" : "before";
  }
  function getDraggedOrder(dropTargetId, sourceOrder = activeSectionPhotos, placement = "before") {
    ...
    return [...remainingPhotos.slice(0, insertIndex), ...movingPhotos, ...remainingPhotos.slice(insertIndex)];
  }
  function previewDraggedPhoto(event, dropTargetId) {
    ...
    const nextOrder = getDraggedOrder(dropTargetId, activeSectionPhotos, placement);
  ...
  async function reorderDraggedPhoto(event, dropTargetId) {
    event.preventDefault();
    const placement = getDropPlacement(event);
    const nextOrder = getDraggedOrder(dropTargetId, activeSectionPhotos, placement) || activeSectionPhotos;
    const movingCount = selectedPhotoIds.includes(draggedPhotoId) ? selectedPhotoIds.length : 1;
    setDraggedPhotoId(null);
    setDragOverPhotoId(null);
    if (!nextOrder.length) return;
    if (await persistPhotoOrder(nextOrder, { optimistic: false })) flash(movingCount > 1 ? "Selected photos moved." : "Photo order updated.");
  }
  ...
  function renderCoverTemplatePreview(style) {
    ...
    const title = <span style={{ color: "#fff", fontFamily: "'Inter', sans-serif", fontSize: 11, fontWeight: 900, letterSpacing: "0.16em", textTransform: "uppercase" }}>Title</span>;
    ...
    if (style.id === "divider") return <div style={{ height: 86, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 3, position: "relative", background: "#fff", padding: 3, boxSizing: "border-box" }}><div style={{ background: imageBackground }} /><div style={{ background: imageBackground, filter: "brightness(0.7)" }} /><div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>{title}</div></div>;
  ...
  function renderDesignPanel() {
    ...
    const renderGridSection = () => <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}><h3 style={{ color: COLORS.white, fontFamily: "'Inter', sans-serif", fontSize: 16, margin: 0 }}>Grid</h3><div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 8 }}>{GRID_STYLES.map((style) => <button key={style} type="button" onClick={() => setGalleryField("grid_style", style)} style={{ ...buttonStyle, color: (gallery.grid_style || "masonry") === style ? COLORS.gold : COLORS.white }}>{style}</button>)}</div></div>;
